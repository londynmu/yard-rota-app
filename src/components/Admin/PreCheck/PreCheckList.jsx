          ...
          <div className="border-t border-gray-200 bg-gray-50 p-4 space-y-4">
            ...
            {(() => {
              ...
              const defectsNoPhoto = allItems.filter(i => {
                if (i.status !== 'repair_needed' && i.status !== 'completed') return false;
                const itemDamages = (sub.precheck_damages || []).filter(d => d.item_id === i.id);
                const hasPhotos = itemDamages.some(d => d.image_urls?.length > 0);
                return !hasPhotos;
              });
