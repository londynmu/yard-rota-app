...
const BrakesManager = () => {
  ...
  // Silent version of fetchBreakData that doesn't show loading spinner
  const fetchBreakDataSilent = useCallback(async () => {
    if (!selectedDate || !selectedShift) return;
    console.log(`[fetchBreakDataSilent] Starting silent fetch for Date: ${selectedDate}, Shift: ${selectedShift}`);
    ...
    try {
      ...
      // Fetch modified standard slot definitions
      let modifiedCapacities = {};
      try {
        const { data: modSlotsData, error: modSlotsErr } = await supabase
          .from('scheduled_breaks')
          .select('std_slot_id, capacity')
          .eq('date', selectedDate)
          .eq('shift_type', selectedShift.toLowerCase())
        ...
        if (!modSlotsErr && modSlotsData && modSlotsData.length > 0) {
          modSlotsData.forEach(mod => {
            if (mod.std_slot_id && mod.capacity !== undefined) {
              modifiedCapacities[mod.std_slot_id] = mod.capacity;
            }
          });
        }
      } catch (modSlotsErr) {
        console.warn("[fetchBreakDataSilent] Error fetching modified standard slot capacities:", modSlotsErr);
      }
      ...
      // Try to fetch custom slots definitions
      let customSlotsData = [];
      try {
        const { data, error } = await supabase
          .from('scheduled_breaks')
        ...
        if (data && data.length > 0) {
          customSlotsData = data.map(slot => ({
            ...
            start_time: slot.break_start_time,
            duration_minutes: slot.break_duration_minutes,
            capacity: slot.capacity || 999,
            break_type: slot.break_type,
            is_custom: true
          }));
        }
      } catch (customSlotError) {
        console.warn("[fetchBreakDataSilent] Error fetching custom slot definitions:", customSlotError);
      }
      ...
      // Fetch existing break assignments
      let processedScheduled = [];
      if (savedAssignments) {
        try {
          processedScheduled = JSON.parse(savedAssignments).map(assignment => ({
          ...
          console.error("[fetchBreakDataSilent] Error parsing sessionStorage assignments:", parseError);
          clearAssignmentCache();
      ...
      if (!savedAssignments) {
        try {
          ...
          if (scheduledData && scheduledData.length > 0) {
            processedScheduled = scheduledData.map(item => {
              const slotIdFromDb = allSlots.find(s => 
                s.start_time === item.break_start_time && 
                s.duration_minutes === item.break_duration_minutes
              )?.id || `custom-${item.id}`;

              return {
                id: item.id,
                user_id: item.user_id,
                slot_id: slotIdFromDb,
                break_date: selectedDate,
                user_name: item.profiles 
                  ? `${item.profiles.first_name} ${item.profiles.last_name}` 
                  : 'Unknown',
                preferred_shift: item.profiles?.shift_preference || null,
                location: item.location || null,
                slot_data: {
                  start_time: item.break_start_time,
                  duration_minutes: item.break_duration_minutes,
          ...
          console.error("[fetchBreakDataSilent] Error fetching scheduled breaks:", err);
        }
      }
      ...
      try {
        const staffQuery = supabase
          ...
          .select(`
            ...
            location,
            profiles:user_id (
              id, first_name, last_name, shift_preference
            )
          `)
          .eq('date', selectedDate)
          .eq('shift_type', selectedShift.toLowerCase());

        const { data: rotaData, error: rotaError } = await staffQuery;
        if (rotaError) throw rotaError;

        if (rotaData && rotaData.length > 0) {
          let processedAvailable = rotaData
          ...
          if (locationFilter) {
            processedAvailable = processedAvailable.filter(staff => staff.location === locationFilter);
          }

          const uniqueStaff = Array.from(
            new Map(processedAvailable.map(s => [s.id, s])).values()
          );
          
          setAvailableStaff(uniqueStaff);
        }
      } catch (err) {
        console.error("[fetchBreakDataSilent] Error processing available staff:", err);
      }
    } catch (err) {
      console.error('Error in fetchBreakDataSilent:', err);
    }
  }, [selectedDate, selectedShift, selectedLocation, getSessionStorageKey, getUnsavedFlagKey, clearAssignmentCache]);
