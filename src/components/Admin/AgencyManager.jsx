...
export default function AgencyManager({ showAddForm = false, setShowAddForm }) {
  ...
  const newAgencyEmailRef = React.useRef(null);
  const editAgencyEmailRef = React.useRef(null);
  const toast = useToast();

  const resizeEmailTextarea = (el) => {
    if (!el) return;
  ...
  useEffect(() => {
    if (showAddForm && newAgencyEmailRef.current) {
      resizeEmailTextarea(newAgencyEmailRef.current);
    }
  }, [showAddForm, newAgency.email]);

  useEffect(() => {
    if (editAgencyId && editAgencyEmailRef.current) {
      resizeEmailTextarea(editAgencyEmailRef.current);
    }
  }, [editAgencyId, editAgencyData.email]);
  ...
  return (
    <div className="space-y-4">
      {/* Add new agency – only when opened from toolbar "+ Add Agency" */}
      {showAddForm && (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 items-start">
            ...
            <div className="min-w-0">
              ...
              <textarea
                ...
                onChange={(e) => {
                  handleInputChange(e);
                  resizeEmailTextarea(e.target);
                }}
                placeholder="Enter agency email"
                rows={1}
                className="w-full min-h-[2.5rem] px-3 py-2 text-sm border border-gray-300 rounded-lg text-charcoal placeholder:text-gray-400 focus:ring-2 focus:ring-charcoal/30 focus:border-charcoal resize-none overflow-hidden align-top"
                style={{ scrollMarginTop: '80px' }}
                disabled={loading}
                ref={newAgencyEmailRef}
              />
            </div>
            <div>
      ...
      {/* Agencies list – closed: only name + action icons; expanded: all details + Save */}
      <div className="space-y-3">
        {loading && agencies.length === 0 ? (
          ...
          agencies.map((agency) => (
            <div
              key={agency.id}
              className={`rounded-xl border border-red-200 bg-red-50/50 shadow-sm ${editAgencyId === agency.id ? 'overflow-visible' : 'overflow-hidden'} ${!agency.is_active ? 'opacity-60' : ''}`}
            >
              {editAgencyId === agency.id ? (
                <div className="p-4 bg-slate-50 space-y-3 border-t border-charcoal/20 overflow-visible">
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 items-start">
                    ...
                    <div className="min-w-0">
                      ...
                      <textarea
                        ...
                        rows={1}
                        className="w-full min-h-[2.5rem] px-3 py-1.5 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-charcoal/30 focus:border-charcoal resize-none overflow-hidden align-top"
                        style={{ scrollMarginTop: '80px' }}
                        disabled={loading}
                        ref={editAgencyEmailRef}
                      />
                    </div>
                    <div>
                ...
                /* Closed card: only agency name + 3 action icons in one line */
                <div className="px-4 py-3 flex flex-nowrap items-center gap-2 min-h-[52px]">
                  <span className={`flex-1 min-w-0 text-sm font-semibold text-charcoal truncate ${!agency.is_active ? 'line-through text-gray-500' : ''}`}>
                    {agency.name}
                  </span>
