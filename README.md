...
- **Implement Multi-level Sorting in Rota View (2024-06-12):** Enhanced the sorting functionality in Weekly Rota View to improve readability. Implemented a three-level sorting system: 1) by start time (earliest first), 2) by end time (if start times match), and 3) alphabetically by staff name (if both times match). This logical ordering ensures that staff entries are displayed in a consistent, predictable manner, making it easier to locate specific staff members and understand shift patterns.
- **Add Rugby/NRC Location Tabs (2024-06-12):** Added separate tabs for Rugby and NRC locations in the My Rota page. The system now filters shifts by location, showing only shifts for the selected location. Staff from different locations are no longer mixed on the same view. Users' tab selection is remembered between sessions using localStorage. This change uses the existing Location Management functionality without requiring database schema changes.
- **Improve Staff Suggestions by End Time (2024-06-11):** Enhanced the staff suggestion logic to prioritize staff members whose preferred end time matches the slot's end time. Added a higher match score for exact end time matches, which ensures staff with matching end time preferences (like 05:45) appear at the top of suggestions while maintaining all existing matching logic.
- **Fix Notification Dropdown Positioning (2024-06-11):** Fixed the notification dropdown UI for proper positioning and appearance. The dropdown now correctly aligns with the notification bell on desktop view and has improved styling with a gradient background. Added responsive design improvements to ensure proper display on both mobile and desktop views.
- **Improve Location Management (2024-06-11):** Enhanced the Location Management interface with icon buttons and delete functionality. Added confirmation dialogs for delete and status toggle actions. Replaced text buttons with intuitive icons for improved mobile display. The interface now matches the Agency Management style with consistent dialog and notification system.
- **Add Icons for Mobile View (2024-06-11):** Replaced text buttons with intuitive icons in the Agency Management interface to improve mobile usability. Used pencil icon for Edit, checkmark/cross icons for Activate/Deactivate, and trash icon for Delete functionality. Added proper ARIA labels and tooltips for accessibility.
- **Improve Agency Management UI (2024-06-11):** Enhanced the agency management interface with modern confirmation dialogs and notifications using React Portal. Replaced native browser alerts with custom styled modals that are consistent with the application theme. Added smooth animations and made all dialogs mobile-friendly. The new interface provides better visual feedback for actions such as deleting, activating, or deactivating agencies.
